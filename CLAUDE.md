...
**Notifications are paced by the previous one closing, not by the clock.** On Windows the interval used to default to 5 seconds — a value for watching the thing work, not for using it. Windows 10+ turns a balloon into a toast, ignores the timeout asked for, and *queues* what it cannot show yet, so a word every five seconds built a backlog that went on appearing minutes after the app had been closed. `ShowWordTick` now skips while a notification is still up, with a 20-second deadline as the safety net: `BalloonTipClosed` is not guaranteed to arrive once the shell has taken over, and the failure worth avoiding is falling silent, not posting a little early. Both ports default to 30 seconds.

**The sets sidebar is the primary way to choose what to learn**, in the list window of both ports; the tray/menu bar entry is a shortcut, not the only route. The user's own words are the first row rather than a special case, which is what the menu never made clear. Removing a set **moves its file into `backups/`** rather than deleting it — same reasoning and same directory as clearing a store. Importing a pack switches to it: downloading a set is asking to learn from it.
...
- **The Swift app has no quiet hours**, only a pause and an interval picker (5 s – 1 h, default 30 s). The .NET app has neither and reads its interval from `appsettings.json`, where it now defaults to the same 30 s.
- **A set imported from an address outside the catalogue cannot be refreshed from the UI.** Its `sourceUrl` is recorded and the merge path exists; only the button is missing.
